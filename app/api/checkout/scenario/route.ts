...
async function methodOptionForRemainder(
  cityId: string,
  lines: CartLine[],
  methodCode: "courier" | "pvz",
) {
  const scenario = await buildScenario({
    cityId,
...
async function buildRemainderResolution(
  cityId: string,
  ...
  const alternativeMethodCodes =
    deliveryMethodCode === "pickup" ? (["courier", "pickup", "pvz"] as const) : (["courier", "pickup", "pvz"] as const).filter(
      (code) => code !== deliveryMethodCode,
    );
